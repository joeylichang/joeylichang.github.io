...
## Ping、Pong
RedisCluster采用的与之前介绍的三种反熵机制（Push、Pull、Push-Pull模式）不同，反熵机制保证经过1-3轮交互之后两个节点的信息相同，RedisCluster使用的是谣言传播机制传播clusterMsgDataGossip数据，即当一个节点有来新信息后，该节点变成活跃状态，并周期性地联系其他节点向其发送新信息。RedisCluster每次只传播10%的节点数据（10%的节点数少于3个传播3个）。RedisCluster内消息的发送与接收通过ping、pong包完成，具体逻辑如下：
