...
##### Doxy对数据节点的探活
Doxy对于数据节点是否故障的决策不会取决于Doxy内部某一个节点，一个Doxy节点判断数据节点故障之后会询问通地域其他节点是否也判断该节点故障，多数达成一致后才会在Doxy集群传播。
...
这种方案不是在proxy层进行同步，而是利用存储节点的binlog写入消息队列。主备集群的sharding规模完全一致，前一种方案中不同的proxy上可能有相同Redis节点的请求，proxy写入消息队列的顺序可能和Redis节点收到的顺序不一致，这种方案可以比较这种问题。
...
PMDK对于事务型写的性能较差，但是使用最底层的读操作性能却几乎与内存出不多，AEP整体表现为读写部队称，上述方案根据通过缓存和异步消费aof来拟补读写的不对称。文章中也之处一些不足，对于高写入场景消费aof的速度跟不上内存落盘的速度，failover之后需要等待内存"预热“之后才能提供服务。
